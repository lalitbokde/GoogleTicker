...
function updateTickerSignalr() {
    ...
    //    url: '/admin/helper/buildticker?&tickerid=' + ticker.data('tickerid'),
    //    success: function (data) {
    //        // alert("refresh");
...
function ticker_update_start() {
    ...
    $tickers.each(function () {
        ...
        //$(this).TickerSetup();
        tickerColor(_ticker);
        tickersetu(_ticker, true);
        //updateTickerSignalr($(this));
        //tickerrefresh($(this));
    });
}

function tickersetu(ticker, animate) {
    //tickerColor(ticker);
    //TickerGlobal = ticker;
    ...
    $.each(ticker_obj[1], function (i, data) {
        ...
        if (ticker_obj[0] % 2 == 1) {
            if (data.OldImpressions != data.Impressions) {
                ...
                arrow = data.OldImpressions > data.Impressions ? '<td class="arrow down"></td>' : '<td class="arrow asc"></td>';
        ...
        if ((ticker_obj[0] >> 1) % 2 == 1) {
            if (data.OldClicks != data.Clicks) {
                arrow = data.OldClicks > data.Clicks ? '<td class="arrow down"></td>' : '<td class="arrow asc"></td>';
            }
        ...
        if ((ticker_obj[0] >> 2) % 2 == 1) {
            if (data.OldConversions != data.Conversions) {
                arrow = data.OldConversions > data.Conversions ? '<td class="arrow down"></td>' : '<td class="arrow asc"></td>';
            }
...
function LoadDataTablesCustomReport() {
...
var toggleColumn = function (colname) {
    ...
    //table.columns('.min-tablet-p').visible(true);
    //table.columns('.desktop').visible(true);
    //table.columns('.all').visible(true);
    table.columns('.never').visible(false);
  //  table.columns((table.column(colname + ":name")[0])).visible(true);
    if (header.hasClass('never'))
    { table.columns((table.column(colname + ":name")[0])).visible(false); }
    else
    { table.columns((table.column(colname + ":name")[0])).visible(true); }
