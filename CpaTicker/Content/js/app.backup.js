...
function runAllForms() {
	...
	if ($.fn.datepicker) {
		$('.datepicker').each(function() {
			...
			var $this = $(this);
			var dataDateFormat = $this.attr('data-dateformat') || 'mm/dd/yy'; //'dd.mm.yy';
...
function setup_widgets_desktop() {
	...
	if ($.fn.jarvisWidgets && $.enableJarvisWidgets) {
		...
		$('#widget-grid').jarvisWidgets({
			...
			//localStorage: true, //njhones commented
			localStorage: false,
...
if($.navAsAjax)
{
    ...
    $(document).on('click', 'nav a[href!="#"], a.doajax', function(e) {
	    e.preventDefault();
	    var $this = $(e.currentTarget);
	    doAjax = 0;
    ...
    $(window).on('hashchange', function() {
        // checkURL();  // njhones commend
        
        // if and doing ajax ok else do nothing
        if (doAjax == 1) {
            checkURL();
        }
        else
...
function loadURL(url, container) {
	...
	$.ajax({
		...
		url : url,
		dataType : 'html',
		//cache : false,//true, // (warning: this will cause a timestamp and will call the request twice)
		beforeSend : function() {
			// cog placed
...
$(document).ready(function () {
    // set Ultral light as default theme
    //$("#smart-style-2").trigger("click");
    //$('.jtextfill').textfill({ debug: true, maxFontPixels: 36 });
    //$('.login-info a').textfill({ debug: true, maxFontPixels: 36 });
});

function pajaxSubmit(f, container, url) {
    $(f).ajaxSubmit({
        type: "POST",
        url: f.action,
        data: $(f).serialize(),
...
function checkURL1(url) {
    ...
    if (url) {
        ...
        // this is the only different with the original chechkurl
        window.location.hash = url;
        ...
        // here i need to call to dashboard ....
...
function runReport(url) {
    ...
    $.rtimer = setTimeout(function () {
        ...
        $.rcolindex = $.rtable.fnSettings().aaSorting[0][0];
        $.rsortdir = $.rtable.fnSettings().aaSorting[0][1];
        //console.log($.rtable.fnSettings().aaSorting);
...
function tickertransform(data) {
    ...
    //console.log(ticker_obj[0]);
    
    $.each(ticker_obj[1], function (i, data) {

        result += '<td><table><tr><td class="c">' + data.CampaignName + '</td></tr>';
        ...
        if ((ticker_obj[0] >> 1) % 2 == 1) {
            if (data.OldClicks != data.Clicks) {
                arrow = data.OldClicks > data.Clicks ? ' <i class="fa fa-sort-asc red"></i>' : ' <i class="fa fa-sort-desc green"></i>';
            }
            aux += '<td class="cl"><div class="val">' + data.Clicks + arrow + '</div><div class="lb">CLICKS</div></td>';
            arrow = '';
        }

        if ((ticker_obj[0] >> 2) % 2 == 1) {
            if (data.OldConversions != data.Conversions) {
                arrow = data.OldConversions > data.Conversions ? ' <i class="fa fa-sort-asc red"></i>' : ' <i class="fa fa-sort-desc green"></i>';
            }            
            aux += '<td class="co"><div class="val">' + data.Conversions + arrow + '</div><div class="lb">CONVERSIONS</div></td>';
            arrow = '';
        }
        ...
        //if (ticker_obj[0] % 2 == 1) {
        //    result += "<span class=\"c l\">Campaign&nbsp;<span class=\"v\">" + data.CampaignName + "</span></span>&nbsp;";
        //}
        //if ((ticker_obj[0] >> 1) % 2 == 1) {
        //    result += "<span class=\"im l\">Impressions&nbsp;<span class=\"v\">" + data.Impressions + "</span></span>&nbsp;";
        //}
        //if ((ticker_obj[0] >> 2) % 2 == 1) {
        //    result += "<span class=\"cl l\">Clicks&nbsp;<span class=\"v\">" + data.Clicks + "</span></span>&nbsp;";
        //}
        //if ((ticker_obj[0] >> 3) % 2 == 1) {
        //    result += "<span class=\"co l\">Conversions&nbsp;<span class=\"v\">" + data.Conversions + "</span></span>&nbsp;";
        //}
        //if ((ticker_obj[0] >> 4) % 2 == 1) {
        //    result += "<span class=\"cs l\">Cost&nbsp;<span class=\"v\">$" + data.Cost.toFixed(2) + "</span></span>&nbsp;";
        //}
        //if ((ticker_obj[0] >> 5) % 2 == 1) {
        //    result += "<span class=\"pf l\">Revenue&nbsp;<span class=\"v\">$" + data.Revenue.toFixed(2) + "</span></span>&nbsp;";
        //}
        //if (result != "") {
        //    result += "<span class=\"d l\">|</span>&nbsp;";
        //}
...
function setTickerColors() {
    $('.ticker .c').css('color', $.cpcolor);  // test with find more optimous it will going to confict with setcolor on the edit page
    $('.ticker .im').css('color', $.imcolor);
    $('.ticker .cl').css('color', $.ckcolor);
    $('.ticker .co').css('color', $.cvcolor);
    $('.ticker .cs').css('color', $.ctcolor);
    $('.ticker .pf').css('color', $.rvcolor);
}

// doAjax on the links inside rtables
