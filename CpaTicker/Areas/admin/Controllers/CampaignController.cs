...
namespace CpaTicker.Areas.admin.Controllers
{
    ...
    public class CampaignController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.IsAffiliate = repo.GetCurrentUser().AffiliateId.HasValue;
        ...
        public ActionResult Details(int id, int? id2) // campaignid, urlid
        {
            ...
            if (!user.AffiliateId.HasValue) // is admin
                ...
                ViewBag.Affiliates = new List<Affiliate>() { repo.GetAffiliate(user.AffiliateId.Value, user.CustomerId) }.Select(
                    a => new SelectListItem { Text = String.Format("{0} - {1}", a.AffiliateId, a.Company), Value = a.AffiliateId.ToString() }).OrderBy(x => x.Value);
            ...
            // var urls = repo.GetCampaignURLs(campaign.Id);    // Previous Urls based on CampaignID

            var urls = repo.GetCustomerPage(user.CustomerId);  //newly added Pages based on customerID
            ...
            //ViewBag.MultipleURLs = urls.Count() > 1; // Prvious condition to set defaul 1 url

            ViewBag.MultipleURLs = true;  //Currently you must have to select one page
            ...
            ViewBag.Categories = repo.GetPageCategories().ToList().Select(t => new SelectListItem { Text = t.Name, Value = t.Id.ToString() });
        ...
        public ActionResult Create(CampaignCreateVM model, string[] ct)
        {
            if (ModelState.IsValid)
            {
                ...
                model.Campaign.CookieExpirationInDays = model.Campaign.CookieExpirationInDays == 0 ? null : model.Campaign.CookieExpirationInDays;
            ...
            //    Text = string.Format("{0} - {1} - {2}", a.Campaign.CampaignId, a.Campaign.CampaignName, a.PreviewId)
            //});
            ViewBag.ParentURLId = repo.GetCustomerPage(model.Campaign.CustomerId).AsEnumerable().Select(a => new SelectListItem
            {
                Value = a.Id.ToString(),
                Text = string.Format("{0} - {1} - {2}", a.Id, a.Name, a.PreviewUrl)
            });
        ...
        public ActionResult Edit(Campaign model, string[] ct)
        {
            ...
            campaign.CookieExpirationInDays = model.CookieExpirationInDays == 0 ? null : model.CookieExpirationInDays;
        ...
        public ActionResult AffiliatePayout(int id)
        {
            ...
            ViewBag.Campaign = campaign;
            var overrideaffiliate = repo.GetAffiliatPayoutCampaign(id);
            int userid = repo.GetCurrentUserId();
            ViewBag.user = repo.UserProfile().SingleOrDefault(u => u.UserId == userid);
            return View(overrideaffiliate.AsQueryable());

        }
        public ActionResult CreateAffiliatePayout(int id)
        {
            //var action = repo.FindAction(id);
            var customerID = repo.GetCurrentCustomer().CustomerId;
            //if (action == null || repo.IsHidden(action.Campaign))
        ...
        public ActionResult EditAffiliatePayout(int id)
        {
            var AffiliatePayout = repo.FindAffiliatePayout(id);
            //var action = repo.FindAction(overridepayout.ActionID);
            var customer = repo.GetCurrentCustomer();
            //if (action == null || repo.IsHidden(action.Campaign))
            //{
            ...
            //}
            //ViewBag.Action = action;
            var campaign = repo.FindCampaign((int)AffiliatePayout.CampaignID);
            if (campaign == null || repo.IsHidden(campaign))
            {
                return HttpNotFound();
            }
            ...
            ViewBag.AffiliateID = new SelectList(repo.GetAffiliateByCustomer(customer.CustomerId), "AffiliateID", "Company", AffiliatePayout.AffiliateID);
            ViewBag.ActionID = new SelectList(campaign.Actions, "Id", "Name", AffiliatePayout.ActionID);
            ViewBag.DDLPayout = AffiliatePayout.PayoutType;
            ViewBag.DDLRevenue = AffiliatePayout.RevenueType;
            return View(AffiliatePayout);
        }

        public ActionResult AddAffiliateOverride(string ActionID, string CampaignID, string AffiliateID, string _PayoutType, string Payout, string PayoutPercent, string _RevenueType, string Revenue, string RevenuePercent)
        {
            int CustomerId = repo.GetCurrentCustomerId();
            var CampaignId = repo.addAffiliateOverrideCampaign(ActionID, CampaignID, AffiliateID, _PayoutType, Payout, PayoutPercent, _RevenueType, Revenue, RevenuePercent);

            return Json(CampaignId);
            //return RedirectToAction("OverridePayout", new { id = Convert.ToInt32(ActionID) });
        }

        public ActionResult UpdateAffiliateOverride(string OverrideID, string ActionID, string CampaignID, string AffiliateID, string _PayoutType, string Payout, string PayoutPercent, string _RevenueType, string Revenue, string RevenuePercent)
        {
            int CustomerId = repo.GetCurrentCustomerId();
            var CampaignId = repo.updateAffiliateOverrideCampaign(Convert.ToInt32(OverrideID), ActionID, CampaignID, AffiliateID, _PayoutType, Payout, PayoutPercent, _RevenueType, Revenue, RevenuePercent);
