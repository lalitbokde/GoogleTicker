...
namespace CpaTicker.Areas.admin.Controllers
{
    ...
    public class HelperController : Controller
    {
        ...
        public PartialViewResult RowDetailsClicksLogs(string id, DateTime? id2 = null)
        {
            var up = repo.GetCurrentUser();
            var customer = repo.GetCurrentCustomer(up.CustomerId);
            ...
            var fdate = new DateTimeOffset(id2.Value.Ticks, tzi.GetUtcOffset(DateTime.Now)).UtcDateTime;
            var tdate = fdate.AddHours(1).AddMilliseconds(-1);
            var detail = repo.ClicksLogs(id, fdate, tdate, up.CustomerId, up.AffiliateId);
        ...
        public ActionResult CustomTimeZone()
        {
            int userId = repo.GetCurrentUserId();
            ViewBag.user = repo.UserProfile().SingleOrDefault(u => u.UserId == userId);
            var Result = repo.getTimeZoneList(userId);
            IEnumerable<AddCustomTimeZone> TimeZones = Result.Select(_model => new AddCustomTimeZone
            {
                DisplayID = _model.DisplayID,
                DisplayName = _model.DisplayName,
                dstEnd = _model.dstEnd,
                offsetHour = _model.offset / 60,
                offsetMinute = _model.offset - ((int)_model.offset / 60) * 60,
                dstoffsetHour = _model.dstoffset / 60,
                dstoffsetMinute = _model.dstoffset - ((int)_model.dstoffset / 60) * 60,
                dstStart = _model.dstStart,
                ID = _model.ID,
                IsdstSupport = _model.IsdstSupport,
                UserID = _model.UserID,
            });
            return PartialView("_TimeZoneListPartial", TimeZones);
        }
        ...
        public ActionResult BuildTicker(DateTime? fromdate, DateTime? todate, int tickerid, string timezone)
        {
            ...
            todate = todate ?? DateTime.Today.AddDays(1).AddSeconds(-1);
            ...
            {
                var AssignedAffiliates = repo.GetUserAffiliates(up.UserId, up.CustomerId).Select(u => u.AffiliateId).ToArray();
                data = data.Where(u => AssignedAffiliates.Contains(u.AffiliateId)).ToList();
            }
            /////////////////////////////////
            ...
            var AffiliateArray = ticker.TickerElements.Where(u => u.Affiliate != null).Select(u => u.Affiliate.AffiliateId).Distinct().ToArray();
            var CampaignArray = ticker.TickerElements.Where(u => u.Campaign != null).Select(u => u.Campaign.CampaignId).Distinct().ToArray();
            foreach (var item in ticker.TickerElements)
            {
                ...
                else if (item.CampaignId.HasValue)
                {
                    ev = (//from e in ((IEnumerable<TickerItem>)ddata).Where(c => c.CampaignId == item.Campaign.CampaignId)
                          //from e in data.Where(c => c.CampaignId == item.Campaign.CampaignId && !AffiliateArray.Contains(c.AffiliateId))
                ...
                else
                {
                    ev = (from e in data.Where(c => c.AffiliateId == item.Affiliate.AffiliateId)
                          ...
                          //(from e in data.Where(c => c.AffiliateId == item.Affiliate.AffiliateId && !CampaignArray.Contains(c.CampaignId))
                ...
                if (ev != null)
                {
                    if (ev.Clicks > 0 || ev.Conversions > 0 || ev.Impressions > 0)
                    { datafilter.Add(ev); }
        ...
        public ActionResult AllAffiliates(bool id) // showall
        {
            ...
            if (id)
            {
                o = repo.GetCustomerAffiliates(customerid).Select(a => new { AffiliateId = a.AffiliateId, Company = a.Company }).OrderBy(u => u.AffiliateId);
            }
            else
            {
                o = repo.GetCustomerActiveAffiliates(customerid).Select(a => new { AffiliateId = a.AffiliateId, Company = a.Company }).OrderBy(u => u.AffiliateId);
            }
        ...
        public ActionResult DeleteRedirectPage(int id)
        {
            ...
            repo.DeleteRedirectPage(ru);
            return RedirectToAction("index", "redirect", new { id = ru.PAGEId });
        }
