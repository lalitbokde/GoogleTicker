...
namespace CpaTicker.Areas.signalra.Controllers
{
    ...
    public class SettingsController : BaseController
    {
        ...
        public ActionResult EditUser(int id)
        {
            ...
            ViewBag.Affiliates = repo.GetCustomerActiveAffiliates(user.CustomerId);
            ViewBag.User = user;
        ...
        public ActionResult EditUser(EditUserVM model)
        {
            ...
            ViewBag.Affiliates = repo.GetCustomerActiveAffiliates(customerid);
            return View(model);
        }
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    using (var scope = new TransactionScope())
                    {
                        ...
                        if (model.HasAPIKey && model.SendEmail)
                        {
                            // send email
                            CPAHelper.SendAPIKeyMail(userprofile);
                        }
        ...
        public ActionResult SetPermissions(int id)
        {
            ...
            ViewBag.Campaigns = from c in repo.Campaigns().Where(c => c.CustomerId == user.CustomerId).AsEnumerable()
        ...
        public ActionResult SetPermissions(int id, string[] views, string[] views1, int[] campaigns)
        {
            ...
            ViewBag.Campaigns = from c in repo.Campaigns().Where(c => c.CustomerId == up.CustomerId).AsEnumerable()
